...
def train(args, teacher, student, generator, device, optimizer, epoch, number_epochs):
    ...
    for i in range(args.epoch_itrs):
        ...
        if i % args.log_interval == 0:
            myprint(f'Train Epoch: {epoch}/{number_epochs} [{i}/{args.epoch_itrs} ({100*float(i)/float(args.epoch_itrs):.0f}%)]\tG_Loss: {loss_G.item():.6f} S_loss: {loss_S.item():.6f}')
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=2048, metavar='N',help='input batch size for training (default: 256)')
    ...
    parser.add_argument('--dataset', type=str, default='cifar100', choices=['MNIST','cifar10', 'cifar100'], help='dataset name (default: cifar10)')
    ...
    parser.add_argument('--ckpt', type=str, default='checkpoint/teacher/teacher_cifar10.pt')
    ...
    num_classes = 100 if args.dataset in ['cifar100'] else 10
    args.num_classes = num_classes
    # args.ckpt = f'./checkpoint/defense/defense_{args.dataset}_resnet18.pth'
    ...
    args.ckpt = f'./ensemble/{args.dataset}.pth'
    teacher = SoftVotingEnsemble(models_loaded)
    ...
    student = get_classifier(args, args.student_model, pretrained=False, num_classes=args.num_classes)
    ...
    for epoch in range(1, number_epochs + 1):
        ...
        train(args, teacher=teacher, student=student, generator=generator, device=device, optimizer=[optimizer_S, optimizer_G], epoch=epoch, number_epochs=number_epochs)
        # Test
